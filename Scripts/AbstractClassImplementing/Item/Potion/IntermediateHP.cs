...
public class IntermediateHP : PotionItem
{
    PotionItemData intermediateHP;  // 중급 체력 회복 물약 아이템 데이터
    ...
    {
        intermediateHP = ScriptableObject.Instantiate(Resources.Load<PotionItemData>("GameData/Item/Potion/IntermediateHPData"));
    }

    // 해당 인벤토리 아이템의 데이터 반환
    public override InventoryItemData GetInventoryItemData()
    {
        return intermediateHP;
    }
    ...
    {
        // 묶음 생성 후 아이템 데이터 세팅
        IntermediateHP newBunndle = gameObject.AddComponent<IntermediateHP>();
    ...
    {
        if (!UsePossible()) throw new System.Exception(intermediateHP.ItemName + " 사용 불가");

        Debug.Log(intermediateHP.ItemName + " 아이템 사용");

        // 재사용 대기시간 초기화 및 세팅
        cooldownTimeMax = intermediateHP.CooldownTime;
        cooldownTime = cooldownTimeMax;
        isCooldownTime = true;

        // 아이템 효과 처리
        // 아이템의 회복 퍼센트 수치만큼 플레이어의 현재 체력 회복 (회복 수치의 소수점은 버림처리)
        PlayerManager.instance.CurrentHp += Mathf.CeilToInt(PlayerManager.instance.MaxHp * intermediateHP.RecoveryPercentage / 100.0f);

        // 아이템 개수 감소
        intermediateHP.CurrentCount--;

        // 해당 슬롯의 아이템의 개수가 0이 되었을 경우, 인벤토리 내에서 삭제
        if (intermediateHP.CurrentCount == 0) InventoryManager.instance.FindAndDeleteSlotItem();
    }
    ...
    public override bool UsePossible()
    {
        ...
        if (intermediateHP.CurrentCount >= 1 && !isCooldownTime && PlayerManager.instance.CurrentHp < PlayerManager.instance.MaxHp) return true;
        else return false;
    }
